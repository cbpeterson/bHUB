function [C_save,Z_save,Sig_save] = hubGGM_SSVS(S,n,Sig,V0,V1,lambda,hub,b1,b2,burnin,nmc)

%% Input:
%%   S: p x p cross product matrix:  Y*Y' if Y is a p x n matrix of random sample
%%   n: sample size
%%   Sig: current covariance matrix
...
%%   hub: p x 1 array of hub indicator
%%   b1: prior marginal edge "inclusion probability" for small probability
%%   b2: prior marginal edge "inclusion probability" for large probability
...
%%% sample Sig and C = inv(Sig)        
    for i = 1:p
      ...
      C(i,i) = gam+c;
		...
		v1 = V1(ind_noi, i);
		m = (hub(i) + hub > 0);
 		pii = b1 * (1 - m(ind_noi)) + b2 * m(ind_noi);
